...
```{r figure_kv_01}
  plotly_kv( tb$kv_b, tb$kv_d, tb$kv_e, dn, tb$zvs, glue("Standrd ",tb$description_en))
```
...
```{r}
  tb.kv <- tibble(position = seq(10,100,by=5)) %>% 
    mutate(kv = drm_LL3(position, tb$kv_b, tb$kv_d, tb$kv_e)*kv_value(dn, tb$zvs)/3.6,
           zv = zv_function(position, tb$kv_b, tb$kv_d, tb$kv_e, tb$zvs),
           position = position/100)

  tb.kv %>%
    gt() %>% 
    tab_header(title    = md("Kv and Zeta Value"),
               subtitle = md("depending on valve position")) %>%
    cols_label(position = md("Valve Position"),
               kv = md("Kv (l/s)"),
               zv = md("Zv (-)")) %>% 
    fmt_percent(columns = position, decimals = 0) %>% 
    fmt_number(columns = c(kv, zv), decimals = 1) %>% 
