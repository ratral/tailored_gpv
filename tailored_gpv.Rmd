...
```{r}
  ...
  p <- plot_ly(tb.system, x = ~position) %>%  
    add_trace(y = ~kv.valve, name = 'trace 0', mode = 'lines',
               hovertemplate = h.template) %>% 
    add_trace(y = ~kv.syst,  name = 'trace 0', mode = 'lines',
               hovertemplate = h.template) %>%
...
```{r}
  ...
  p <- plot_ly(tb.system, x = ~position) %>%  
    add_trace(y = ~kv.valve.rel, name = 'Valve',  mode = 'lines', hovertemplate = h.template) %>% 
    add_trace(y = ~kv.syst.rel,  name = 'System', mode = 'lines', hovertemplate = h.template) %>% 
    add_trace(y = ~position,     name = 'Linear', mode = 'lines', hovertemplate = h.template) %>%
